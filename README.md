...
The ready-to-use plugin files can be found in the `tlp-plugin` directory.
...
- Create new GitHub release using your new version number as the "Tag version". Use the exact version number, don't include a prefix `v`. See here for an example: <your-release-url>
...
The compiled plugin files are provided in the `tlp-plugin` folder. To install the plugin into your vault, copy the entire `tlp-plugin` directory to `VaultFolder/.obsidian/plugins/` so that the folder name matches the plugin ID (`tlp-plugin`).
